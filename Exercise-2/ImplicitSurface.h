...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		Eigen::Vector3d centered_points = _x - this->m_center; 
		return centered_points.transpose() * centered_points - std::pow(this->m_radius,2);
	}
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		Eigen::Vector3d centered_points = _x - this->m_center; 
		return std::pow(centered_points.transpose() * centered_points + std::pow((this->m_radius), 2) - std::pow(this->m_a,2),2)
		- 4*std::pow(this->m_radius,2)*(std::pow(centered_points[0],2) + std::pow(centered_points[1],2)) ;
	}
